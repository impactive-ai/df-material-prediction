...
def _preprocessing_arima(df_input, target, test_length, future_length):
...
def _predicting_arima(df_expanded, df_x, test_set_length, future_length):
    model_name = "ARIMA"
    ...
    what_to_predict = df_x.columns

    for target_idx in tqdm(what_to_predict):
        df_train, df_test, df_future = _preprocessing_arima(
            df_expanded, target_idx, test_set_length, future_length
        )
        # Model
        model = auto_arima(
            df_train["y"],
...
def _preprocessing_prophet(df_input, target, valid_length, test_length, future_length):
    df_input = df_input[["dt", target]]
    ...
    # Validation Set
    df_valid = df_input.iloc[
        -(valid_length + test_length + future_length) : -(test_length + future_length)
    ]
...
def _predicting_prophet(
    ...
    for target_idx in tqdm(what_to_predict):
        ...
        df_train, df_valid, df_test, df_future = _preprocessing_prophet(
            df_tmp, target_idx, valid_set_length, test_set_length, future_length
        )
...
def _preprocessing(df_input, target, test_length, future_length):
...
def _preprocessing_lstm(x, y, test_length, future_length):
    # Train Set
...
def _predicting_lstm(df_expanded, df_x, test_length, future_length):
    model_name = "LSTM"
    ...
    for target_idx in tqdm(what_to_predict):
        ...
        df_train, df_test, df_future = _preprocessing(
            df_tmp, target_idx, test_length, future_length
        )
        ...
        # Data Split
        x_train, y_train, x_test, y_test, x_future, y_future = _preprocessing_lstm(
            x, y, test_length, future_length
        )
        ...
        # Prediction
        pred = _predicting_sliding_window(
            model, scaler, x_test, (len(x_test) + len(x_future)), window_size
        )
        ...
        df_test_ts = pd.DataFrame(
            df_expanded.iloc[-(test_length + future_length) : -(future_length),]["dt"]
        )
        df_test_ts = df_test_ts.reset_index(drop=True)
        ...
        df_future_ts = df_future_ts.reset_index(drop=True)
...
def _predicting_bi_lstm(df_expanded, df_x, test_length, future_length):
    model_name = "BI-LSTM"
...
def _predicting_gru(df_expanded, df_x, test_length, future_length):
    model_name = "GRU"
...
def _finding_best(df_expanded, df_x):
    ...
    df_all_metric_result = df_all_metric_result.reset_index(drop=True)

    for target_idx in what_to_predict:
        # Best Metric
        ...
        df_expanded = df_expanded.merge(
            df_tmp_update[["dt", target_idx]], on="dt", how="left"
        )
        df_expanded[target_idx] = df_expanded[f"{target_idx}_y"].combine_first(
            df_expanded[f"{target_idx}_x"]
        )
        df_expanded = df_expanded.drop([f"{target_idx}_x", f"{target_idx}_y"], axis=1)

    # Saving Result as files
    save_path = f"./output/FeaturesForecasting/"
    if not os.path.exists(save_path):
        os.makedirs(save_path)
