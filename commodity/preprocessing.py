...
def making_lag_features(df_target, target_name):
    ...
    for i in range(1, lag_num):
        df_lag_result[f"delta_lag{i}_lag{i + 1}"] = (
            df_lag_result[f"lag_{i + 1}"] - df_lag_result[f"lag_{i}"]
        ) / df_lag_result[f"lag_{i + 1}"]

    # fill "mean_window"
    df_lag_result["mean_window_3"] = df_lag_result[["lag_1", "lag_2", "lag_3"]].mean(
        axis=1
    )
    df_lag_result["mean_window_6"] = df_lag_result[lag_columns].mean(axis=1)
    # fill "std_window"
    df_lag_result["std_window_3"] = df_lag_result[["lag_1", "lag_2", "lag_3"]].std(
        axis=1
    )
